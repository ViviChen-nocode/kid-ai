...
const FlipbookReader = ({ currentPage, onPageChange }: FlipbookReaderProps) => {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 overflow-auto">
      {/* Book container */}
      <div 
        className={`relative w-full max-w-5xl ${isMobile ? 'max-w-md' : ''} flex items-center justify-center transition-transform duration-300 origin-center`}
        style={{ transform: `scale(${zoom})` }}
      >
        {/* Pages */}
        <div className={`flex ${isCoverOrBack && !isMobile ? 'justify-center' : 'justify-center gap-1'} w-full`}>
          {isMobile ? (
